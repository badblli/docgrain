...
def _require_version(version_id: str) -> DocumentVersion:
    version = next((v for v in fixtures.VERSIONS if v.id == version_id), None)
    if version is None:
...
def retry(version_id: str, payload: RetryRequest) -> RetryResponse:
    ...
    _require_version(version_id)
    job = next((j for j in fixtures.JOBS if j.document_version_id == version_id), None)
    if job is None:
